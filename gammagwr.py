...
class GammaGWR:
    ...
    # eq 2 par 3.1
    # decadimento esponenziale normalizzato a intergrale 1
    ...
    def init_network(self, ds, random, **kwargs) -> None:
        ...
        self.depth = self.num_context + 1  # K. depth of recursion
    ...
    def add_node(self, b_index) -> None:
        ...
        # it basically does the avg between BMU weights (and context) and the input (and context) ((input + bmu weights) / 2)
    ...
    def update_weight(self, index, epsilon) -> None:
        delta = np.dot(
            (self.g_context - self.weights[index]), (epsilon * self.habn[index]))
    ...
    ###########################################################
    # keeps history of neuron activations for certain classes #
    # with a "histogram graph". every cycle adds a_inc to the #
    # the column (label) of the BMU histogram and subtract    #
    # a_dec to everything else                                #
    ###########################################################
    ...
    #########################################################
    # reset edges between BMU and second best unit SBU to 1 #
    # (remember, edges decay if not used).                  #
    # Also reset the age of the edge (edges can be cut if   #
    # not used frequently enough)                           #
    # ----                                                  #
    # If a new node is added it links it to both BMU & SBU  #
    #########################################################
    ...
    def train_ggwr(self, ds, epochs, a_threshold, beta, l_rates) -> None:
        ...
        self.a_threshold = a_threshold  # activity threshold, exp(-'distance between input and BMU')
        ...
        for epoch in range(0, self.max_epochs):
            ...
            for iteration in range(0, self.samples):
                ...
                # activity of network is defined as distance (euclidean) between input and BMU
